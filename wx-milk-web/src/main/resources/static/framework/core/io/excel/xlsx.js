...
define(function (require, exports, module) {
    (function make_xlsx(XLSX) {
        ...
        var Factory = require('./worker.factory.js');
        var current_codepage = 1200, current_cptable;
        if (typeof M !== "undefined" && typeof R !== 'undefined') {
            if (typeof cptable === 'undefined') cptable = R('./dist/cpexcel');
...
//var MSCSZ = 4096; /* Mini Stream Cutoff Size */
        ...
        if (typeof R !== 'undefined' && typeof M !== 'undefined' && typeof DO_NOT_EXPORT_CFB === 'undefined') {
            M.exports = CFB;
        }
        function isval(x) {
        ...
        if (typeof exports !== 'undefined') {
            if (typeof M !== 'undefined' && M.exports) {
                if (has_buf && typeof jszip === 'undefined') jszip = R('js' + 'zip');
                if (typeof jszip === 'undefined') jszip = R('./js' + 'zip').JSZip;
                _fs = R('f' + 's');
            }
        }
        var attregexg = /([\w:]+)=((?:")([^"]*)(?:")|(?:')([^']*)(?:'))/g;
...
//	if(s + length != blob.l) throw "TxO " + (s + length) + ", at " + blob.l;
        ...
        function get_cell_style(styles, cell, opts) {
            ...
            var key = "_styles";
            if( styles.hasOwnProperty(key) && styles[key][z]){
                return styles[key][z].i;
            }
            if(!styles.hasOwnProperty(key)){
                styles[key] = {};
            }
            styles[len] = styles[key][z] = {
                numFmtId: z,
                fontId: 0,
                fillId: 0,
                borderId: 0,
        ...
        function write_ws_xml_cols(ws, cols) {
            ...
            for (var i = 0; i != cols.length; ++i) {
                ...
                if (typeof col.hidden !== 'undefined')
                    p.hidden = col.hidden;
        ...
        function write_ws_xml_data(ws, opts, idx, wb) {
            ...
            if ( range.e.r > 1000) { //async &&
                var data = {
                    XLSX: XLSX,
                    cols:cols,
                    ws: ws,
                    opts: {cellDates:opts.cellDates,bookSST:opts.bookSST},
                    idx: idx,
                    wb: wb,
                    range: range
                };
                return write_ws_xml_data_async(opts,data);
            }
        ...
        /**
         * 由于worker不能做线程同步，共享变量先处理掉。String值是全局的
         * @param ws
         * @param opts
         * @param range
         * @param cols
         */
        function setStringIndex(ws,opts,range,cols){
            var C, R,ref,rr;
            for (R = range.s.r; R <= range.e.r; ++R) {
                rr = encode_row(R);
                for (C = range.s.c; C <= range.e.c; ++C) {
                    ...
                    if (ws[ref] === undefined) continue;
                    var cell = ws[ref];
                    if(cell.t != "n" && cell.t != "d" && cell.t != "b" && cell.t != "e"){
                        cell.sv =  get_sst_id(opts.Strings, cell.v);
                    }
                    cell.ss = get_cell_style(opts.cellXfs, cell, opts);
                }

            }
        }
        function write_ws_xml_data_async(opts,data) {
            var factory = new Factory('/resources/js/core/io/excel/excel.work.js', XLSX.poolsize);
            var pageSize = 500;
            var count = Math.ceil(data.range.e.r / pageSize);
            ...
            function getData(index) {
                ...
                if( pageIndex == 0 )
                    p.pageStart = range.s.r;
                else
                    p.pageStart = pageIndex * pageSize;
                p.pageEnd = Math.min(data.range.e.r+1, (pageIndex + 1) * pageSize);
                for (var R = p.pageStart; R < p.pageEnd; ++R) {
                    var  rr = encode_row(R);
                    for (var C = range.s.c; C <= range.e.c; ++C) {
                        var  ref = data.cols[C] + rr;
                        ws[ref] = data.ws[ref];
                        delete data.ws[ref];
                    }
                }
            ...
            setStringIndex(data.ws,opts,data.range,data.cols);
            ...
            var callback = function(datas){
                ...
                if( pageIndex < count){
                    factory.post(getData).then(function(){
                        var o= [];
        ...
        function write_ws_xml(idx, opts, wb, callback) {
            ...
            if (ws['!ref'] !== undefined) {
                write_ws_xml_data(ws, opts, idx, wb).then(function (data) {
                    rdata = data;
        ...
        function parse_wb_xml(data, opts) {
            ...
            data.match(tagregex).forEach(function xml_wb(x) {
                ...
                switch (strip_ns(y[0])) {
                    ...
                    /* 18.2.16 oleSize CT_OleSize ? (ref Rd) */
                    case '<oleSize':
                        break;
        ...
        function write_ws(data, name, opts, wb, callback) {
            return (name.substr(-4) === ".bin" ? write_ws_bin : write_ws_xml)(data, opts, wb, callback);
        }
...
// 2.3.2
        ...
        function parse_ods(zip, opts) {
            if (typeof M !== "undefined" && typeof R !== 'undefined' && typeof ODS === 'undefined') ODS = R('./od' + 's');
        ...
        function write_zip(wb, opts, callback) {
            if (wb && !wb.SSF) {
                wb.SSF = SSF.get_table();
            }
            if (wb && wb.SSF) {
            ...
            for (rId = 1; rId <= wb.SheetNames.length; ++rId) {
                (function(rId){
                    var f = "xl/worksheets/sheet" + rId + "." + wbext;
                    write_ws(rId - 1, f, opts, wb, function(d){
                        zip.file(f,d);
                        ct.sheets.push(f);
                        add_rels(opts.wbrels, rId, "worksheets/sheet" + rId + "." + wbext, RELS.WS);
                        doneIndex +=1;
                        if( wb.SheetNames.length -1 <= doneIndex){
                            func();
                        }
                    });
                })(rId);
        ...
        function write_zip_type(wb, opts, callback) {
            var o = opts || {};
            write_zip(wb, o, function(z){
                var r;
                switch (o.type) {
                    case "base64":
                        r= z.generate({type: "base64"});
                        break;
                    case "binary":
                        r= z.generate({type: "string"});
                        break;
                    case "buffer":
                        r= z.generate({type: "nodebuffer"});
                        break;
                    case "file":
                        r= _fs.writeFileSync(o.file, z.generate({type: "nodebuffer"}));
                        break;
                    default:
                        throw new Error("Unrecognized type " + o.type);
                }
        ...
        function writeSync(wb, opts, callback) {
            var o = opts || {};
            switch (o.bookType) {
                case 'xml':
                    return write_xlml(wb, o);
                default:
                    write_zip_type(wb, o,callback);
            }
        }

        function writeFileSync(wb, filename, opts,callback) {
            var o = opts || {};
            o.type = 'file';
            o.file = filename;
        ...
        XLSX.write_ws_xml_cell = write_ws_xml_cell;
